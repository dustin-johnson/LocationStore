 ...
 * 
 * <p>
 * A state is a <code>State</code> object and must implement <code>processMessage</code> and optionally <code>enter/exit/getName</code>. The enter/exit methods are equivalent to the construction and
 * destruction in Object Oriented programming and are used to perform initialization and cleanup of the state respectively. The <code>getName</code> method returns the name of the state the default
 * implementation returns the class name it may be desirable to have this return the name of the state instance name instead. In particular if a particular state class has multiple instances.
 * </p>
 * 
 * <p>
 * When a state machine is created <code>addState</code> is used to build the hierarchy and <code>setInitialState</code> is used to identify which of these is the initial state. After construction the
 * programmer calls <code>start</code> which initializes and starts the state machine. The first action the StateMachine is to the invoke <code>enter</code> for all of the initial state's hierarchy,
 * starting at its eldest parent. The calls to enter will be done in the context of the StateMachines Handler not in the context of the call to start and they will be invoked before any messages are
 * processed. For example, given the simple state machine below mP1.enter will be invoked and then mS1.enter. Finally, messages sent to the state machine will be processed by the current state, in our
 * simple state machine below that would initially be mS1.processMessage.
 * </p>
 * <code>
mP1
/ \
 ...
 * <p>
 * After the state machine is created and started, messages are sent to a state machine using <code>sendMessage</code> and the messages are created using <code>obtainMessage</code>. When the state
 * machine receives a message the current state's <code>processMessage</code> is invoked. In the above example mS1.processMessage will be invoked first. The state may use <code>transitionTo</code> to
 * change the current state to a new state
 * </p>
 * 
 * <p>
 * Each state in the state machine may have a zero or one parent states and if a child state is unable to handle a message it may have the message processed by its parent by returning false or
 * NOT_HANDLED. If a message is never processed <code>unhandledMessage</code> will be invoked to give one last chance for the state machine to process the message.
 * </p>
 * 
 * <p>
 * When all processing is completed a state machine may choose to call <code>transitionToHaltingState</code>. When the current <code>processingMessage</code> returns the state machine will transfer to
 * an internal <code>HaltingState</code> and invoke <code>halting</code>. Any message subsequently received by the state machine will cause <code>haltedProcessMessage</code> to be invoked.
 * </p>
 * 
 * <p>
 * If it is desirable to completely stop the state machine call <code>quit</code> or <code>abort</code>. These will call <code>exit</code> of the current state and its parents, call
 * <code>onQuiting</code> and then exit Thread/Loopers.
 * </p>
 * 
 * <p>
 * In addition to <code>processMessage</code> each <code>State</code> has an <code>enter</code> method and <code>exit</exit> method which may be overridden.
 * </p>
 * 
 * <p>
 * Since the states are arranged in a hierarchy transitioning to a new state causes current states to be exited and new states to be entered. To determine the list of states to be entered/exited the
 * common parent closest to the current state is found. We then exit from the current state and its parent's up to but not including the common parent state and then enter all of the new states below
 * the common parent down to the destination state. If there is no common parent all states are exited and then the new states are entered.
 * </p>
 * 
 * <p>
 * Two other methods that states can use are <code>deferMessage</code> and <code>sendMessageAtFrontOfQueue</code>. The <code>sendMessageAtFrontOfQueue</code> sends a message but places it on the front
 * of the queue rather than the back. The <code>deferMessage</code> causes the message to be saved on a list until a transition is made to a new state. At which time all of the deferred messages will
 * be put on the front of the state machine queue with the oldest message at the front. These will then be processed by the new current state before any other messages that are on the queue or might
 * be added later. Both of these are protected and may only be invoked from within a state machine.
 * </p>
 * 
 * <p>
 ...
 * <p>
 * After starting mS5 the list of active states is mP0, mP1, mS1 and mS5. So the order of calling processMessage when a message is received is mS5, mS1, mP1, mP0 assuming each processMessage indicates
 * it can't handle this message by returning false or NOT_HANDLED.
 * </p>
 * 
 * <p>
 * Now assume mS5.processMessage receives a message it can handle, and during the handling determines the machine should change states. It could call transitionTo(mS4) and return true or HANDLED.
 * Immediately after returning from processMessage the state machine runtime will find the common parent, which is mP1. It will then call mS5.exit, mS1.exit, mS2.enter and then mS4.enter. The new list
 * of active states is mP0, mP1, mS2 and mS4. So when the next message is received mS4.processMessage will be invoked.
 * </p>
 * 
 * <p>
...
public class StateMachine {
    ...
    public static class LogRec {
         ...
         * @param orgState
         *            is the first state the received the message but did not processes the message.
         */
     ...
     * 
     * The class maintains a list of log records including messages recently processed. The list is finite and may be set in the constructor or by calling setSize. The public interface also includes
     * size which returns the number of recent records, count which is the number of records processed since the the last setSize, get which returns a record and add which adds a record.
     */
    private static class LogRecords {

        private static final int DEFAULT_SIZE = 20;

        private final Vector<LogRec> mLogRecords = new Vector<LogRec>();
         ...
         * 
         * @param maxSize
         *            number of records to maintain at anyone time.
         */
        ...
        /**
         * @return the information on a particular record. 0 is the oldest record and size()-1 is the newest record. If the index is to large null is returned.
         */
    ...
    private static class SmHandler extends Handler {
        ...
        /**
         * Handle messages sent to the state machine by calling the current state's processMessage. It also handles the enter/exit calls and placing any deferred messages back onto the queue when
        ...
        private void performTransitions() {
            ...
            while (mDestState != null) {
                ...
                /**
                 * Since we have transitioned to a new state we need to have any deferred messages moved to the front of the message queue so they will be processed before any other messages in the
                 * message queue.
                 */
        ...
        private final void moveDeferredMessageAtFrontOfQueue() {
            /**
             * The oldest messages on the deferred list must be at the front of the queue so start at the back, which as the most resent message and end with the oldest messages at the front of the
             * queue.
             */
         ...
         * 
         * This is found by searching up the destState's ancestors for a state that is already active i.e. StateInfo.active == true. The destStae and all of its inactive parents will be on the
         * TempStateStack as the list of states to enter.
         * 
         * @return StateInfo of the common ancestor for the destState and current state or null if there is no common parent.
         */
        private final StateInfo setupTempStateStackWithStatesToEnter(State destState) {
            /**
             * Search up the parent list of the destination state for an active state. Use a do while() loop as the destState must always be entered even if it is active. This can happen if we are
     ...
     * 
     * this function can also be called inside the enter function of the previous transition target, but the behavior is undefined when it is called mid-way through a previous transition (for example,
     * calling this in the enter() routine of a intermediate node when the current transition target is one of the nodes descendants).
     * 
     * @param destState
    ...
    /**
     * transition to halt state. Upon returning from processMessage we will exit all current states, execute the onHalting() method and then for all subsequent messages haltedProcessMessage will be
     * called.
     */
    ...
    /**
     * Defer this message until next state transition. Upon transitioning all deferred messages will be placed on the queue and reprocessed in the original order. (i.e. The next state the oldest
    ...
    /**
     * This will be called once after a quit message that was NOT handled by the derived StateMachine. The StateMachine will stop and any subsequent messages will be ignored. In addition, if this
     * StateMachine created the thread, the thread will be stopped after this method returns.
     */
